function [count edges mid loc] = DT_histcn(X, varargin)
...
% count = zeros(size(hasdata));
% if isempty(AccumData) 
%     AccumData = ones(size(X,1),1);
% end
   


if ~isempty(AccumData)    
    count = uint16(accumarray(loc(hasdata,:), AccumData(hasdata), sz, Fun{:}));
    %count = accumarray(loc(hasdata,:), AccumData(hasdata), sz);
else
    count = uint16(accumarray(loc(hasdata,:), 1, []));
end
